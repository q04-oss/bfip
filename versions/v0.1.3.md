...
### 7b.1 Purpose
Cleared status is an optional elevation above attested status that unlocks specific platform capabilities restricted to users who have passed a criminal background check. It is not required for core platform access — it is a voluntary step taken by users who want access to elevated features.

### 7b.2 Requirements
A user may pursue cleared status only after reaching `attested` status. Cleared status requires:
- Active attested soultoken in good standing
- Criminal background check passed

### 7b.3 Criminal background check
The criminal background check screens against national criminal records databases in the user's jurisdiction. It uses the identity data confirmed by Stage 1 Stripe Identity verification.

Box Fraise stores only the pass/fail signal, provider name, external check ID, and a response integrity hash. The underlying criminal record data is never stored.

### 7b.4 Cleared soultoken
Upon passing the criminal background check, a cleared soultoken is issued. The cleared soultoken:
- Is a separate credential from the attested soultoken
- References the attested soultoken that qualified the user
- Is independently revocable — a user can lose cleared status without losing attested status
- Expires after 12 months (matching criminal check validity period)
- Cannot be renewed by presence events — renewal requires a new criminal background check

### 7b.5 What cleared status unlocks
Cleared status unlocks platform capabilities defined by Box Fraise that require a higher trust threshold. Examples include:
- Access to events restricted to cleared users
- Direct communications channels only available to cleared users
- Physical spaces or experiences requiring cleared status

The specific capabilities unlocked by cleared status are defined by platform configuration, not by the protocol.

### 7b.6 Revocation
A cleared soultoken may be revoked if:
- The criminal background check result is rescinded
- The underlying attested soultoken is revoked
- The check expires without renewal

Revocation of a cleared soultoken does not affect attested status. The user retains core platform access.

### 7b.7 Jurisdiction
Criminal background checks are jurisdiction-specific. The protocol requires that the check covers the user's country of residence as confirmed by Stage 1 identity verification. Multi-jurisdiction checks are at the discretion of the platform.
...
### 18.1 Purpose
The trust registry is a public, read-only record of Box Fraise's cryptographic signing keys and registered BFIP-accepting businesses. It enables any business to verify a BFIP credential offline without querying Box Fraise directly.

### 18.2 What the trust registry contains
- Box Fraise's current and historical Ed25519 public keys for soultoken signature verification
- The current BFIP protocol version
- A list of businesses registered as BFIP-accepting nodes, their locations, and the tier they require
- Beacon locations registered on the network

### 18.3 What the trust registry does not contain
- User records of any kind
- Query history
- Verification transaction records

The trust registry is infrastructure, not a database. Box Fraise does not broker relationships between users and businesses. It issues credentials once and steps back.

### 18.4 Credential verification model
Box Fraise is a credential issuer, not a directory. Verification works as follows:

1. User presents their soultoken display code or signed attestation token to a business
2. Business retrieves Box Fraise's public key from the trust registry
3. Business verifies the Ed25519 signature on the credential locally
4. No query to Box Fraise is required
5. Box Fraise is not involved in the verification transaction

This model mirrors how a passport works. The issuing authority (Box Fraise) is not present at every verification. The credential is self-contained and cryptographically verifiable by anyone with the public key.

### 18.5 Business registration
A business registers in the trust registry by:
- Completing the business onboarding protocol (Section 12)
- Specifying which credential tier they require for their use case
- Publishing their location as a BFIP-accepting node

Registration is a one-time act. Ongoing participation is maintained through the delivery and quality assessment relationship.

### 18.6 Offline verification
Because verification requires only the public key and the credential, it works without internet connectivity. The trust registry is cacheable. A business that has cached Box Fraise's public key can verify credentials indefinitely without a server connection.

This is the foundation for the Box Fraise Mesh Protocol (BFMP), which defines how verified nodes communicate and relay events offline. See https://github.com/q04-oss/bfmp.

---

## 19. Business participation lifecycle

### 19.1 Overview
A business on the Box Fraise platform is not a customer of a service. It is a node in a verified physical network. The relationship begins with strawberry deliveries and grows into operational infrastructure.

### 19.2 Onboarding
1. A verified attested user registers a business (Section 12)
2. Business soultoken is issued
3. First delivery is scheduled
4. Beacon is installed during the first delivery visit
5. Business is listed in the trust registry as a BFIP-accepting node

### 19.3 Ongoing relationship
Every delivery visit maintains the relationship:
- Strawberry boxes delivered — commercial transaction
- Quality assessment completed — standards enforced
- Attestations performed for eligible users — trust generated
- Support interactions resolved — operational relationship deepened

The delivery cadence is driven by order volume. More orders means more frequent visits. More frequent visits means more attestation opportunities for users at that business.

### 19.4 Expanded services
As the network matures, businesses can access expanded services through their participation:

| Service | Tier required | Description |
|---------|--------------|-------------|
| Customer onboarding | Standard | Replace KYC with BFIP standard credential |
| Staff verification | Professional | Verify employee credentials at hiring |
| Compliance verification | Professional | Regulatory compliance via credential |
| Employment verification | Trusted | Background-checked staff credentials |
| Facility access control | Cleared | Restrict access to cleared credential holders |

### 19.5 The commercial trajectory
A business that begins with strawberry deliveries and a beacon ends up using Box Fraise as operational infrastructure for hiring, compliance, customer onboarding, and access control. The entry point is food. The destination is critical business infrastructure.

### 19.6 Mesh participation
A business beacon is a node in the Box Fraise mesh network. As the mesh protocol matures, business beacons relay presence events, validate neighboring beacons, and enable offline verification. The value of a business's beacon participation increases as the network grows — Metcalfe's law applied to physical infrastructure.

The mesh protocol is defined separately as the Box Fraise Mesh Protocol (BFMP). See https://github.com/q04-oss/bfmp.
...
| background_check_passed | All required checks passed for current stage |
| background_check_failed | One or more checks failed |
| background_check_review_required | Provider returned inconclusive result |
| cleared_status_granted | All five checks passed, cleared soultoken issued |
| cleared_status_revoked | Cleared soultoken revoked independently of attested | sanctions and identity fraud checks passed |
